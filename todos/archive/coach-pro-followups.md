...
- [ ] **L19** Extract pure fns from `handleCoachChat`:
      - `calculateWeeklyRate(weightLogs)` → `server/services/weight-trend.ts`
      - `truncateNotebookToBudget(entries, maxChars)` → new `server/services/notebook-budget.ts`
      - `hashCoachCacheKey(userId, contextHash, content)` → new helper or
        inlined but tested
